...
def test_define_variable():
    prog = parse("(defvar x footype)")
    assert evaluate(prog) == UNIT


def test_var_access():
    prog = parse("( (defvar x footype) (set x 3)  x )")
    assert evaluate(prog) == 3

    prog = parse("( (defvar x footype) (set x 4) (set x 3) 3)")
    assert evaluate(prog) == 3
...
def test_defining_functions():
    ...
    prog = parse("( (defvar x foobar) (set x 1000) (defun add-3 (un int) ((x un int)) (+ x 3))  (add-3 7) )")
    assert evaluate(prog) == 10

    prog = parse("( (defvar x foobar) (set x 1000) (defun add-3 (un int) ((x un int)) (+ x 3))  (add-3 7) x)")
    assert evaluate(prog) == 1000

    prog = parse("( (defvar x foobar) (set x 1000) (defun add-3 (un int) ((x un int)) (+ x 3))  (add-3 x))")
...
def test_if_statement():
    ...
    prog = parse("(  (defvar x foobar) "
                 "    (set x 1000)  "
                 "    (defun add-3 (un int) ((x un int)) (+ x 3))"
                 "    (if (= 3 4) ((defvar z foobar) (set z 10) (add-3 x)) (add-3 (add-3 z)))"
                 ")")
    ...
    prog = parse("(  (defvar x foobar) "
                 "    (set x 1000)  "
                 "    (defun add-3 (un int) ((x un int)) (+ x 3))"
                 "    (if (= 3 4) (add-3 x) (((defvar z foobar) (set z 10))  (add-3 (add-3 z)))) z"
                 ")")
    with pytest.raises(VariableUndefinedError):
        evaluate(prog)
...
def test_while_statement():
    prog = parse("( (defvar x foobar) (set x 0) "
                 "  (defvar y foobar) (set y 10)"
    ...
    prog = parse("( (defvar x foobar) (set x 0) "
                 "  (defvar y foobar) (set y 10)"
                 "  (while (< x -4) ((defvar z foobar) (set z 10)) "
    ...
    prog = parse("( (defvar x foobar) (set x 0) "
                 "  (defvar y foobar) (set y 10)"
                 "  (while (< x -4) ((defvar z foobar) (set z 10)) "
                 "               ( (defvar 1 foobar) (set 1 10) (set y (+ y 1)) (set x (+ x 1)) y)  ) q)")
    with pytest.raises(VariableUndefinedError):
        evaluate(prog)
